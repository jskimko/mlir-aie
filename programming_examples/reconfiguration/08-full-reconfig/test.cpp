...
//
// Rung 08 -- full-reconfig host (core + DMA + switch, SIMULTANEOUSLY). Config i
// is the shared packet_flow host-fed walk pipeline placed on tile_i: host input
// -> compute tile (out = in + K(i)) -> host output, both legs walked. ALL THREE
// knobs vary at once -- the walk (switch), the add constant K(i) = 10 + i
// (core), and the output DMA gather order dims(i) = PATTERNS[(i-1)%4] (DMA).
// Each config remaps the SAME input to a DISTINCT output (a distinct layout AND
// a distinct magnitude, on a distinct tile), so the N correct outputs are each
// other's negative control -- a stuck / last-only / never-reconfigure mechanism
// reads the wrong slice (or times out on a route miss) and fails the per-config
// gate.
//
// The per-config payloads (the access pattern and the add constant) are
// DECOUPLED from the config index i (spec section 5): i is only the symbol
// suffix in the artifact; the pattern is PATTERNS[(i-1) % PATTERNS.size()] and
// the constant is K(i) = 10 + i, selected separately. So a mechanism that used
// the suffix as the payload would apply the wrong layout/magnitude and fail.
// PATTERNS and k_for() below mirror gen.py exactly -- the two must agree.
//
...
// element-distinct input in[e] = e this is out[m] = order_i[m] + (10 + i) -- a
// stuck dispatch cannot read as a valid result, and each config's output is a
// distinct permutation-plus-offset of the others.
//
// Switch observability: config i places the pipeline ONLY on tile_i, so data
// reaches the dest only if BOTH legs re-pointed to tile_i. A switch miss
// surfaces as a TIMEOUT (the abandoned tile's quiesced core is not
// re-producing), which still fails the gate -- a timeout on this rung is a
// switch/route symptom, not a write32<->ctrlpkt delivery regression.
//
...
// method-tagged artifacts that coexist in build/. All three methods now fold
// onto ONE overlay ELF (build/overlay_<N><tag>.elf, built by aiecc
// --get-full-elf --reconfig-method=<m>) with a shared main:init entry (when
// the method has one) plus main:config_1..main:config_N, so the host
// dispatch path is IDENTICAL across methods -- only the tag-derived
// has_init / ctrlpkt-dummy-arg flags below vary:
//   loadpdi (METHOD=loadpdi, tag "_loadpdi")  OOB non-persistent full reload:
//         each main:config_i keeps its own un-expanded load_pdi, so
//         dispatching it is a true full-PDI reload (the hw_context is created
//         ONCE at init on the shared ELF, not per config -- the reload cost is
//         the load_pdi inside each config_i dispatch, not a host-side
//         hw_context recreate). No main:init / no ctrlpkt dummy arg: a
//         load_pdi is a full reset every time it is applied, so a shared init
//         standup would be redundant -- any main:config_i already resets on
//         its own (aiecc's splitMultiConfigEntry no longer synthesizes one for
//         this method). This is the correctness oracle. Note: main:init used
//         to double as a benchmark warmup (arming the hw_context before the
//         first timed dispatch); with no init, the first timed
//         main:config_1 dispatch is now cold.
//   write32 (METHOD=write32, tag "_write32")  OOB persistent, no-overlay
//         direct-write: reset-free by default (no main:init / load_pdi at
//         all). WITHRESET=1 restores the old @empty init reset (tag
//         "_write32_wr"), so the host dispatches main:init for that variant.
//         No ctrlpkt dummy arg either way.
//   ctrlpkt (METHOD=ctrlpkt, tag "_ctrlpkt", default)  IB persistent: a
//         resident control-packet overlay, armed once via main:init (a
//         load_pdi of the overlay itself), delivers each config in-band
//         (baked .ctrldata). The only method whose kernel signature carries
//         the extra (inert) ctrlpkt-slot arg. The only method that always has
//         a main:init (it stands up the shared resident overlay).
//
...
// The core payload: config i's copy loop adds K(i) = 10 + i. VARIES per config
// (core is one of the three reconfigured classes here). Offset from i so the
// payload is decoupled from the symbol suffix. MUST match gen.py's addk().
...
// simulates the nested loop to recover the per-position source index. Kept in
// lockstep with gen.py by construction (same list); a drift there without a
// matching edit here fails the gate.
...
// reconfigures per config. loadpdi's main:config_i keeps its own un-expanded
// load_pdi (a true full-PDI reload); write32's is a no-overlay direct-write;
// ctrlpkt's is a baked control-packet delivery against the resident overlay.
static int run_overlay(const Config &cfg) {
  ...
  ReconfReport rpt;
  rpt.unit = "full-reconf";
  rpt.header =
      std::string("full reconfiguration (rung 08, ") + method_desc + ") -- " +
