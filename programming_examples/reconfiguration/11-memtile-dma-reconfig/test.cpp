...
//
// Rung 11 -- memtile-dma-reconfig host, the MEM-TILE variant of rung 03. Config
// i reconfigures the DMA ACCESS PATTERN ONLY, but the reconfigured channel is
// the MEM TILE (0,1)'s MM2S (the link-output objectFifo that relays
// shim(0,0) -> mem-tile(0,1) -> compute(0,2)), not the compute tile: the
// mem-tile-producer's gather order is pattern (i-1) % PATTERNS, distinct per
// cycled config; the core (copy + fixed add CORE_ADD), the route, the tiles,
// and the transfer LENGTH are held constant. Each config remaps the SAME input
// to a DISTINCT output LAYOUT, so the N distinct correct outputs are each
// other's negative control -- a stuck / last-only / never-reconfigure
// mechanism reads a slice with the wrong layout and fails the per-config gate.
//
// The per-config payload (the access pattern) is DECOUPLED from the config
// index i (spec section 5): i is only the symbol suffix in the artifact; the
// pattern is PATTERNS[(i-1) % PATTERNS.size()], so a mechanism that used the
// suffix as the payload would apply the wrong layout and fail. PATTERNS and
// CORE_ADD below mirror gen.py exactly -- the two must agree.
//
// The oracle: the mem-tile relay gathers the flat input buffer in config i's
// visit order and delivers it to the compute tile, which writes
// out_local[k] = in[order_i[k]] + CORE_ADD in flat receive order (k = 0..n-1);
// the output leg is direct and contiguous, so out[m] = in[order_i[m]] +
// CORE_ADD -- the same end-to-end formula as rung 03, now produced by the
// mem-tile's gather instead of the compute tile's. With the element-distinct
// input in[e] = e this is out[m] = order_i[m] + CORE_ADD -- a stuck dispatch
// cannot read as a valid result, and each pattern's output is a distinct
...
// The core payload: the constant every config's copy loop adds. HELD CONSTANT
// across configs (the core is not the reconfigured class -- the DMA access
// pattern is). MUST match gen.py.
static constexpr int CORE_ADD = 100;
...
// out[m] = out_local[order[m]] = in[order[m]] + CORE_ADD.
...
// reconfigures the resident mem-tile DMA per config.
static int run_overlay(const Config &cfg) {
  ...
  auto matches = [&](int A) {
    ...
    for (int m = 0; m < n; m++)
      if (out_map[m] != (DTYPE)(in[order[m]] + CORE_ADD))
        return false;
    return true;
  };
  ...
  ReconfReport rpt;
  rpt.unit = "dma-reconf";
  rpt.header = std::string("memtile dma reconfiguration (rung 11, ") +
