...
#
# Core+switch combo rung 07 generator. Emits ONE single-config module for config i of the shared
# packet_flow HOST-FED walk pipeline (common/walk_pipeline.py): host input -> compute tile
# (out = in + K(i)) -> host output, with the compute tile WALKED across the grid and BOTH pipeline
# legs expressed as explicit aie.packet_flow. TWO classes reconfigure per config (core + switch);
# the DMA gather is HELD identity, so this isolates the core+switch pair of the full protocol.
#
...
#   DMA    : HELD -- the output MM2S gather order is contiguous (identity) in every config.
#
...
def design(i, nsrc, n=N_ELEM, dev="npu2"):
    ...
    """
    return walk_pipeline.design(i, nsrc, n=n, k=addk(i), dims="", dev=dev)
