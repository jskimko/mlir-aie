...
//
// Rung 17 -- whole-array-matmul. The real
// basic/matrix_multiplication/whole_array design (a single AIE core computing C
// = A @ B via the external kernels.mm matmul + zero kernels) folded verbatim
// into ONE self-contained overlay ELF (build/overlay_1<tag>.elf): a shared
// "main:init" entry (the overlay's one aiex.npu.load_pdi) plus a load_pdi-free
// "main:sequence" entry, this config's control packets baked into its
...
int main(int argc, char **argv) {
  ...
  ReconfReport rpt;
  rpt.unit = "whole-array-matmul";
  rpt.header = "whole-array-matmul (rung 17) -- " + std::to_string(Ncfg) +
               " config(s) baked in ONE overlay ELF (main:sequence.." +
  ...
  rpt.lat_detail = [Ncfg] {
    return "main:sequence run total / " + std::to_string(Ncfg);
  };
  ...
  std::vector<xrt::ext::kernel> k_cfg; // main:sequence..N, one dispatch each
  ...
  return reconf_bench(
      ...
      [&](Bench &b, int i) {
        ...
        b.require(
            ...
            "whole-array-matmul mismatch", [] { sched_yield(); });
      });
}
