...
//
// Rung 10 -- input-coexist. One self-contained overlay ELF
...
// section.
//
// This rung is a coexistence fixture, not a reconfiguration sweep. Phase 1
// (--inputs 1): ONE circuit input leg (shim MM2S data) plus the IB overlay's
// control ingress (shim MM2S control) -- 2 of the shim's 2 MM2S channels, so
// it fits; the host binds arg0 (the in-place data buffer), arg1/arg2 a zeroed
// dummy, and checks out = in + 1. Phase 2 (--inputs 2, both circuit) is the
// wall (3 MM2S demanded on a 2-MM2S shim). Phase 3 (--inputs 2 + PKTIN=1)
// packetizes in1 so its shim MM2S becomes a data allocation control
// time-shares -> demand drops to 2 and it fits; the host binds three real
// buffers (in0=arg0, in1=arg1, out=arg2) and checks out = in0 + in1.
//
// --inputs selects the emitted design at gen.py/Makefile time; the host also
// reads it to pick the matching oracle (1 = out = in + 1; 2 = out = in0 + in1).
//
...
//             60000)
//     --inputs I  host input legs (1 or 2); selects the oracle and, at build
//                 time, the emitted design.
...
int main(int argc, char **argv) {
  ...
  ReconfReport rpt;
  rpt.unit = "input-coexist";
  rpt.header = "input-coexist (rung 10) -- " + std::to_string(N) +
  ...
  if (inputs == 2) {
    // 2-input coexistence (Phase 3): in0 (arg0, circuit shim MM2S), in1 (arg1,
    // packet shim MM2S -- control time-shares this channel), out (arg2, S2MM).
    // Oracle: out = in0 + in1.
