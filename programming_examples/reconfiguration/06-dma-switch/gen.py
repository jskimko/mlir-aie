...
#
# DMA+switch combo rung 06 generator. Emits ONE single-config module for config i of the shared
# packet_flow HOST-FED walk pipeline (common/walk_pipeline.py): host input -> compute tile
# (out = in + CORE_ADD) -> host output, with the compute tile WALKED across the grid and BOTH
# pipeline legs expressed as explicit aie.packet_flow. TWO classes reconfigure per config (DMA +
# switch); the core is HELD, so this isolates the DMA+switch pair of the full-reconfig protocol.
#
#   switch : the walk -- config i places the pipeline on tile_i and re-points both legs
#            (walk_pipeline's src_col/src_row). The self-clear teardown (unconditional for write32/ctrlpkt) tears down the abandoned
...
#            varied per config.
#   core   : HELD -- the add constant is fixed at CORE_ADD (the core is NOT reconfigured here).
#
...
# needs self-clear (unconditional for write32/ctrlpkt) so each config tears down its own route before the next.
...
CORE_ADD = 100  # rung 06 holds the core add constant (DMA+switch reconfigure)
...
def design(i, nsrc, n=N_ELEM, dev="npu2"):
    ...
    return walk_pipeline.design(
        i, nsrc, n=n, k=CORE_ADD, dims=dims_to_stream(pattern_for(i)[1]), dev=dev
    )
