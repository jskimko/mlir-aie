...
//
// Rung 03 -- dma-reconfig host. Config i reconfigures the DMA ACCESS PATTERN
// ONLY: the output objectFifo's producer-side gather order is pattern (i-1) %
// PATTERNS, distinct per cycled config; the core (copy + fixed add CORE_ADD),
// the route, the tiles, and the transfer LENGTH are held constant. Each config
// remaps the SAME input to a DISTINCT output LAYOUT, so the N distinct correct
// outputs are each other's negative control -- a stuck / last-only /
// never-reconfigure mechanism reads a slice with the wrong layout and fails the
// per-config gate.
//
...
//
// The oracle: the core writes out_local[k] = in[k] + CORE_ADD in flat order (k
// = 0..n-1), then the output DMA gathers out_local in config i's visit order
// into the contiguous DDR buffer, so out[m] = in[order_i[m]] + CORE_ADD. With
// the element-distinct input in[e] = e this is out[m] = order_i[m] + CORE_ADD
// -- a stuck dispatch cannot read as a valid result, and each pattern's output
// is a distinct permutation of the others.
//
...
// reconfigures the resident DMA per config.
static int run_overlay(const Config &cfg) {
  ...
  rpt.unit = "dma-reconf";
  rpt.header =
      std::string("dma reconfiguration (rung 03, ") + method_desc + ") -- " +
